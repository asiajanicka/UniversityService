...
public class GradeDAO implements IGradeDAO {
    ...
    public Optional<Grade> getEntityById(long id) {
        ...
        try {
            ...
            try (PreparedStatement prepStmt = con.prepareStatement(GET_GRADE)) {
    ...
    public int updateEntity(Grade entity) {
        ...
        try {
            ...
            try (PreparedStatement prepStmt = con.prepareStatement(UPDATE_GRADE)) {
    ...
    public Optional<Grade> createEntity(Grade entity) {
        ...
        try {
            con = ConnectionPool.getInstance().getConnection();
            logger.debug(String.format(CONNECTED_DB + desc, entity));
            try (PreparedStatement prepStmt = con.prepareStatement(CREATE_GRADE, Statement.RETURN_GENERATED_KEYS)) {
                prepStmt.setInt(1, entity.getValue());
    ...
    public int removeEntity(long id) {
        ...
        try {
            con = ConnectionPool.getInstance().getConnection();
            logger.debug(String.format(CONNECTED_DB + desc, id));
            try (PreparedStatement prepStmt = con.prepareStatement(REMOVE_GRADE)) {
                prepStmt.setLong(1, id);
                int result = prepStmt.executeUpdate();
                logger.debug(String.format(EXECUTED_QUERY + desc, id));
                return result;
    ...
    public List<Grade> getAllGradesByStudentId(long studentId) {
        ...
        try {
            ...
            try (PreparedStatement prepStmt = con.prepareStatement(GET_GRADES_BY_STUDENT_ID)) {
    ...
    public List<Grade> getAllGradesBySubjectId(long subjectId) {
        ...
        try {
            con = ConnectionPool.getInstance().getConnection();
            logger.debug(String.format(CONNECTED_DB + desc, subjectId));
            try (PreparedStatement prepStmt = con.prepareStatement(GET_GRADES_BY_SUBJECT_ID)) {
                ...
                con.close();
                logger.debug(String.format(CLOSED_CON_DB + desc, subjectId));
            }
        } catch (SQLException e) {
            logger.error(String.format(NOT_CONNECT_DB + desc, subjectId), e);
            e.printStackTrace();
        }
        return grades;
    }
    ...
    public int bindGradeToStudentId(long gradeId, long studentId) {
        ...
        try {
            con = ConnectionPool.getInstance().getConnection();
            logger.debug(String.format(CONNECTED_DB + desc, gradeId, studentId));
            try (PreparedStatement prepStmt = con.prepareStatement(BIND_GRADE_TO_STUDENT)) {
                prepStmt.setLong(1, studentId);
                ...
                con.close();
                logger.debug(String.format(CLOSED_CON_DB + desc, gradeId, studentId));
            }
        } catch (SQLException e) {
            logger.error(String.format(NOT_CONNECT_DB + desc, gradeId, studentId), e);
            e.printStackTrace();
        }
        return 0;
    }
